    ...
    async activate(params, routeConfig, navigationInstruction) {
        ...
        if (this.data.Items) {
            ...
            for (let item of this.data.Items) {
                ...
                item.PriceDifference = numeral(item.PriceDifference).format();
                item.Percentage = numeral(item.Percentage).format();
                item.LastPurchasePrice = numeral(item.LastPurchasePrice).format();
                item.SupplierPrice1 = numeral(item.SupplierPrice1).format();
                item.SupplierPrice2 = numeral(item.SupplierPrice2).format();
                item.SupplierPrice3 = numeral(item.SupplierPrice3).format();
                item.LastPurchaseDate = moment(item.LastPurchaseDate).format("DD MMMM YYYY");
        }
