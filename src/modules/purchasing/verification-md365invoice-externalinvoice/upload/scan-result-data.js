	...
	_applyEditsToResult() {
		...
		if (this.supportRef) {
			...
			if (poRows && root.PurchaseOrder) {
				if (Array.isArray(root.PurchaseOrder.PurchaseOrder)) root.PurchaseOrder.PurchaseOrder = poRows;
				if (Array.isArray(root.PurchaseOrder.purchaseOrder)) root.PurchaseOrder.purchaseOrder = poRows;
			}
			// DeliveryOrder container
			if (doRows && root.DeliveryOrder) {
				if (Array.isArray(root.DeliveryOrder.DeliveryOrder)) root.DeliveryOrder.DeliveryOrder = doRows;
				if (Array.isArray(root.DeliveryOrder.deliveryOrder)) root.DeliveryOrder.deliveryOrder = doRows;
			}
