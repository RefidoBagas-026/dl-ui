...
import { ro } from '../../../inventory/garment-waste/receipt-waste/template/ro-fabric';
...
export class Item {
  ...
  activate(context) {
    ...
    if(this.data.ExportSalesDOItems){
      const items = this.data.ExportSalesDOItems;
      this.data.roType='RO JOB';
      console.log(items);
      if (items && Array.isArray(items)) {
          for (const item of items) {
              this.selectedRO = {
                  Id: item.costCalculationGarmentId,
                  RO_Number: item.roNumber
              };
              this.selectedROChanged(this.selectedRO);
          }
      }
    }
  }
  onremove(data, event) {
   if (this.context && this.context.context && this.context.context.options && this.context.context.options.header && Array.isArray(this.context.context.options.header.items)) {
      const items = this.context.context.options.header.items;
